...
- **LLM stack:** Google Gemini (`google-genai`) for the lightweight generic
  flow; Claude (via chat, using `agent_prompts/*.txt`) for full crawler-based
  projects
...
2. Screenshots are grouped by module and fed through `agent_prompts/*.txt`
   (one prompt per module) in a Claude chat to produce `manual_sections/*.md`.
...
projects/<name>/             # one folder per fully-crawled software
  ...
  agent_prompts/<module>.txt    # per-module manual-writing prompt for Claude
